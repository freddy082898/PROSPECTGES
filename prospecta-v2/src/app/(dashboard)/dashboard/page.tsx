...
async function getDashboardData(userId: string) {
  try {
    ...
    return { prospects: prospects[0] as any, campaigns: campaigns[0] as any, deals: deals[0] as any, byStatus: byStatus as any[], recent: recent as any[] };
  } catch {
    return null;
  }
}
...
export default async function DashboardPage() {
  ...
  const byStatusList: Array<{ status: string; count: string }> = data?.byStatus ?? [];
  const recentList: Array<{ first_name: string; last_name: string; company: string; score: number }> = data?.recent ?? [];
